...
- Import a feature's modules by path. Do not add `index.ts` barrels. Put supporting UI in `components/` and React hooks in `hooks/` when a feature has more than a handful of files; otherwise keep the files flat.
