...
const sections = ['Staged', 'Changes', 'Untracked'] as const
      ...
      <div className="flex flex-1 flex-col gap-3 p-3">
        {sections.map((section) => (
          <div key={section} className="space-y-1.5">
            ...
            {rows.map((row) => (
              <div key={`${section}-${row}`} className="flex items-center gap-2 px-2 py-1.5">
                <Skeleton className="size-3 rounded-sm" />
                <Skeleton className="h-3 rounded" style={{ width: `${55 + ((row * 19) % 30)}%` }} />
              </div>
            ))}
          </div>
        ))}
