...
function statusFor(file: string): string {
  return repo.git('status', '--porcelain', '--', file).slice(0, 2)
}

beforeEach(async () => {
  repo = createRepoFixture({ prefix: 'rebase-hunk-new-' })
  repoDir = repo.path
  repo.writeLines('tracked.txt', ['base'])
...
describe('hunk staging on files git does not track yet', () => {
  it('stages the hunk of an untracked file into the index', async () => {
    repo.writeLines('brand-new.txt', ['alpha', 'beta', 'gamma'])
    ...
    expect(repo.git('show', ':brand-new.txt')).toBe('alpha\nbeta\ngamma\n')
    expect(statusFor('brand-new.txt')).toBe('A ')
    expect(repo.read('brand-new.txt')).toBe('alpha\nbeta\ngamma\n')
  })

  it('stages the hunk of an untracked file inside a subdirectory', async () => {
    repo.writeLines('nested/deep/brand-new.txt', ['one', 'two'])
    ...
    expect(repo.git('show', ':nested/deep/brand-new.txt')).toBe('one\ntwo\n')
  })

  it('stages the hunk of an untracked file in a repository with no commits', async () => {
    await runOp(closeRepo(repoDir))
    repo.cleanup()
    repo = createRepoFixture({ prefix: 'rebase-hunk-empty-' })
    repoDir = repo.path
    repo.writeLines('first.txt', ['hello'])
    await runOp(openRepo(repoDir))
    ...
    expect(repo.git('show', ':first.txt')).toBe('hello\n')
  ...
  it('stages one hunk of a partially staged new file and leaves the other unstaged', async () => {
    ...
    repo.writeLines('multi.txt', lines)
    ...
    repo.writeLines('multi.txt', edited)
    ...
    expect(repo.git('show', ':multi.txt')).toContain('line 1 EDITED')
    expect(repo.git('show', ':multi.txt')).not.toContain('line 36 EDITED')
  ...
  it('ships the untracked fallback text as the patch, matching the hunks beside it', async () => {
    repo.writeLines('patch-check.txt', ['alpha', 'beta'])
  ...
  it('unstages the hunk of a staged new file back to untracked', async () => {
    repo.writeLines('staged-new.txt', ['alpha', 'beta'])
    ...
    expect(repo.read('staged-new.txt')).toBe('alpha\nbeta\n')
