  ...
  it('rejects an unresolvable repo path with a typed GitError for every guarded op', async () => {
    const program = Effect.gen(function* () {
      ...
      const calls: Effect.Effect<unknown, unknown>[] = [
        ...
        client.mergeBranch({ repoPath: invalidRepo, refKind: 'local', fullPath: 'main' }),
        ...
        client.stashPop({ repoPath: invalidRepo, index: 0, expectedOid: 'abc123' }),
        client.stashApply({ repoPath: invalidRepo, index: 0, expectedOid: 'abc123' }),
        client.stashDrop({ repoPath: invalidRepo, index: 0, expectedOid: 'abc123' }),
