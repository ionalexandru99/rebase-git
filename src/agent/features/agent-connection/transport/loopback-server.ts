...
function monitorServerErrors(server: Server, logger: AgentLogger): Effect.Effect<never> {
  return Effect.forever(
    Effect.callback<Error>((resume) => {
      const onError = (error: Error) => resume(Effect.succeed(error))
      server.once('error', onError)
      return Effect.sync(() => server.off('error', onError))
    }).pipe(
      Effect.flatMap((error) =>
        logger.write('agent-loopback-server-error', { error: error.message })
      )
    )
  )
}
...
export function serveLoopback(
  ...
  return Effect.gen(function* () {
    const running = yield* Effect.acquireRelease(
      Effect.callback<{ readonly server: Server; readonly port: number }, AgentLoopbackBindFailure>(
        (resume) => {
          ...
          const absorbPostBindError = () => undefined
          server.once('error', fail)
          server.listen(port, AGENT_LOOPBACK_HOST, () => {
            server.off('error', fail)
            server.on('error', absorbPostBindError)
    ...
    yield* monitorServerErrors(running.server, logger).pipe(Effect.forkScoped)
    return running.port
  })
}
