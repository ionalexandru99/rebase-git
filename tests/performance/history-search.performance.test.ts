...
test("cached metadata search on repository history and 250,000 merge-heavy commits", async ({
  ...
  try {
    ...
    await page.goto(url);
    const measurements = await page.evaluate(async (repositoryCommits) => {
      const storePath =
      ...
      for (const corpus of corpora) {
        ...
        const roots = [
          { name: "main", oid: corpus.commit(0).oid, type: "branch" as const },
        ];
        ...
        for (const text of corpus.queries) {
          ...
          do {
            const result = await searchStoredRepositoryHistory(
              environmentId,
              repositoryId,
              { text, limit: 20, ...(cursor === undefined ? {} : { cursor }) },
            );
          ...
          const measurement = {
            ...
            firstMatchOrEndMilliseconds: fullMilliseconds,
            scans,
            matches,
          };
